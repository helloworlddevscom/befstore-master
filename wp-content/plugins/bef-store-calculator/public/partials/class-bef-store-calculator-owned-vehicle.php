...
if (class_exists('GF_Field')) {
    class bef_store_calculator_owned_vehicle extends GF_Field {
        public $type = 'custom_owned_table';

        private $column_headers = ['ave-mpg' => 'Average MPG*', 'fuel-type' => 'Fuel Type', 'total-annual-miles' => 'Total Annual Miles'];
        ...
        // bef-store-calculator/includes/src/calculators/complexCommute/ownedVehicleFleet.js
        private $classVals = ['car','hybrid','electricVehicle','pickupTruckVan','deliveryTruck','semiBigRig'];

        public function get_form_editor_field_title() {
            return esc_attr__('[C] Owned Table', 'txtdomain');
        }
        ...
        public function get_field_input($form, $value = '', $entry = null) {
            ...
            $table = '<table class="table__owned-vehicle"><tbody><tr>';
            $table .= '<th>' . __('Vehicle Type', 'txtdomain') . '</th>';
            ...
            $counter = 0;
            foreach ($this->choices as $vehicle) {
                $table .= '<tr>';
                $table .= '<td>' . $vehicle['text'] . '</td>';
                foreach (array_keys($this->column_headers) as $col) {
                    if ($col === 'ave-mpg') {
                        if (!in_array( $counter,[1,2] )){
                            $table .= '<td><input class="ownedVehicleMPG ' . $this->classVals[$counter] . '" type="number" size="1" name="input_' . $id . '[]" value="' . $table_value[$vehicle['text']][$col] . '" /></td>';
                        } else {
                            $table .= '<td><input readonly class="ownedVehicleMPG ' . $this->classVals[$counter] . '" type="text" size="1" name="input_' . $id . '[]" value="' . $table_value[$vehicle['text']][$col] . '" /></td>';
                        }
                    }
                    if ($col === 'fuel-type'){
                        if (in_array( $counter,[0] )){
                            $table .= '<td><select class="ownedVehicleFuelType ' . $this->classVals[$counter] . '"  name="input_' . $id . '[]" value="' . $table_value[$vehicle['text']][$col] . '" ><option value="gasoline" selected>Gasoline</option><option value="diesel">Diesel</option></select></td>';
                        }
                        if (in_array( $counter,[1,2] )){
                            $table .= '<td><input readonly class="ownedVehicleFuelType ' . $this->classVals[$counter] . '" type="text" size="1" name="input_' . $id . '[]" value="' . $table_value[$vehicle['text']][$col] . '" /></td>';
                        }
                        if (in_array( $counter,[3,4,5] )){
                            $table .= '<td><select class="ownedVehicleFuelType ' . $this->classVals[$counter] . '"  name="input_' . $id . '[]" value="' . $table_value[$vehicle['text']][$col] .
                                '" ><option value="gasoline" selected>Gasoline</option><option value="diesel">Diesel</option><option value="cng">CNG</option><option value="ethanol">Ethanol</option></select></td>';
                        }
                    }
                    if ($col === 'total-annual-miles'){
                        $table .= '<td><input class="ownedVehicleMiles ' . $this->classVals[$counter] . '" type="number" size="1" name="input_' . $id . '[]" value="' . $table_value[$vehicle['text']][$col] . '" /></td>';
                    }
                }
                $counter++;
                $table .= '</tr>';
        ...
        private function translateValueArray($value) {
            ...
            if (empty($value)) {
                foreach ($this->choices as $row) {
                    foreach (array_keys($this->column_headers) as $col) {
                        if ($col === 'ave-mpg') {
                            $table_value[$row['text']][$col] = $row['value'];
                            $counter++;
                        }
                        if ($col === 'fuel-type') {
                            $table_value[$row['text']][$col] = $row['value'];
        ...
        private function listOutput($value) {
            ...
            foreach ($value as $vehicle => $data) {
                $val =  $data;
                array_push($value_number, array_pop($val));
            }
            ...
            if ($value_sum > 0) {
                ...
                $header = '';
                $header .= '<th class="review_table__header">Vehicle Type</th>';
                ...
                $body = '';
                foreach ($value as $vehicle => $entry) {
                    // Only add entries if there were any requests at all
                    $body .= '<tr><td class="review_table__entry">' . $vehicle . '</td>';
        ...
        public function get_value_entry_list($value, $entry, $field_id, $columns, $form) {
            return __('Enter details to see commute details', 'txtdomain');
        }
    ...
    GF_Fields::register(new bef_store_calculator_owned_vehicle());
}
