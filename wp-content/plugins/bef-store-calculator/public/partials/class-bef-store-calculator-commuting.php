...
/**
 * Code for generating custom gravity form code for custom commuting tables
 *
...
if (class_exists('GF_Field')) {
    class bef_store_calculator_commuting extends GF_Field {
        public $type = 'custom_commuting_table';

        private $column_headers = ['employees' => '# of Employees', 'one-way-commute' => 'Ave 1-Way Commute in Miles'];
        ...
        private $classVals = ['car','hybrid','electricVehicle','trainRailSubway','bus','taxi','ferry'];

        public function get_form_editor_field_title() {
            return esc_attr__('[C] Commute', 'txtdomain');
        }
        ...
        public function get_field_input($form, $value = '', $entry = null) {
            ...
            $table = '<table class="table__commute-vehicle"><tbody><tr>';
            ...
            foreach ($this->choices as $vehicle) {
                ...
                foreach (array_keys($this->column_headers) as $col) {
                    if ($col === 'employees') {
                        $table .= '<td><input class="numEmployees ' . $this->classVals[$counter] . '" type="number" size="1" name="input_' . $id . '[]" value="' . $table_value[$vehicle['text']][$col] . '" /></td>';
                    } else {
                        $table .= '<td><input class="aveCommuteMiles ' . $this->classVals[$counter] . '" type="number" size="1" name="input_' . $id . '[]" value="' . $table_value[$vehicle['text']][$col] . '" /></td>';
                    }
                }
                $counter++;
                $table .= '</tr>';
    ...
    GF_Fields::register(new bef_store_calculator_commuting());
}
