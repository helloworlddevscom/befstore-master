...
if (class_exists('GF_Field')) {
    class bef_store_calculator_personal_vehicle extends GF_Field {
        public $type = 'custom_personal_table';

        private $column_headers = ['ave_mpg' => 'Average MPG*', 'total-annual-miles' => 'Total Annual Miles'];
        ...
        // bef-store-calculator/includes/src/calculators/complexCommute/personalVehicleFleet.js
        private $classVals = ['car','hybrid','electricVehicle','pickupTruckVan'];

        public function get_form_editor_field_title() {
            return esc_attr__('[C] Personal Veh', 'txtdomain');
        }
        ...
        public function get_field_input($form, $value = '', $entry = null) {
            ...
            $table = '<table class="table__personal-vehicle"><tbody><tr>';
                ...
                foreach (array_keys($this->column_headers) as $col) {
                    if ($col === 'ave_mpg') {
                        if (!in_array( $counter,[1,2] )){
                            $table .= '<td><input class="personalVehicleMPG ' . $this->classVals[$counter] . '" type="number" size="1" name="input_' . $id . '[]" value="' . $table_value[$vehicle['text']][$col] . '" /></td>';
                        } else {
                            $table .= '<td><input readonly class="personalVehicleMPG ' . $this->classVals[$counter] . '" type="text" size="1" name="input_' . $id . '[]" value="' . $table_value[$vehicle['text']][$col] . '" /></td>';
                        }
                    } else {
                        $table .= '<td><input class="personalVehicleMiles ' . $this->classVals[$counter] . '" type="number" size="1" name="input_' . $id . '[]" value="' . $table_value[$vehicle['text']][$col] . '" /></td>';
                    }
                }
                $counter++;
                $table .= '</tr>';
    ...
    GF_Fields::register(new bef_store_calculator_personal_vehicle());
}
