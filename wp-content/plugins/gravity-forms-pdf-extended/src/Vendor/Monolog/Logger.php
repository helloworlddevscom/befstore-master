 ...
 */
namespace GFPDF\Vendor\Monolog;

use GFPDF\Vendor\Monolog\Handler\HandlerInterface;
 ...
 */
class Logger implements \Psr\Log\LoggerInterface, \GFPDF\Vendor\Monolog\ResettableInterface
{
    /**
     ...
     * @return $this
     */
    public function pushHandler(\GFPDF\Vendor\Monolog\Handler\HandlerInterface $handler)
    {
    ...
    {
        if (!$this->handlers) {
            $this->pushHandler(new \GFPDF\Vendor\Monolog\Handler\StreamHandler('php://stderr', static::DEBUG));
        }
    ...
    {
        foreach ($this->handlers as $handler) {
            if ($handler instanceof \GFPDF\Vendor\Monolog\ResettableInterface) {
                $handler->reset();
            }
        }
        foreach ($this->processors as $processor) {
            if ($processor instanceof \GFPDF\Vendor\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
