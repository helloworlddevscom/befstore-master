...
class Templates {
	...
	public function save_template( $values ) {
		...
		try {
			...
			if ( $template_id === '0' ) {
				$new_template = fg_fillablepdfs()->api->create_template( $name, $file );
			} else {
				$new_template = fg_fillablepdfs()->api->save_template( $template_id, $name, $file );
			}
	...
	public function validate_template_name( $field, $value = '' ) {
		...
		try {
			...
			$existing_templates = fg_fillablepdfs()->api->get_templates();
