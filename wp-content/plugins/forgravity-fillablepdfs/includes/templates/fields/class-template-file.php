...
class Template_File extends Hidden {
	...
	public function markup() {
		...
		$html .= sprintf(
			'<div class="fillablepdfs-template-info"%1$s>
				<img src="%2$s/images/templates/placeholder.svg" width="100" class="fillablepdfs-template-info__placeholder" alt="%3$s">
				...
				<button type="button" class="fillablepdfs-template-info__action fillablepdfs-template-info__action--remove">%5$s</button>
			</div>',
			$show_upload ? ' style="display:none;"' : '',
			fg_fillablepdfs()->get_base_url(),
			$this->settings->get_field( 'name' )->get_value(),
			rgar( $template, 'pdf_name', '' ),
			esc_html__( 'Remove', 'forgravity_fillablepdfs' )
		);
	...
	public function do_validation( $value ) {
		...
		try {
			...
			// Get file meta.
			$has_fields = fg_fillablepdfs()->api->get_file_meta( $uploaded_file, true );
