		...
		if ( $is_valid ) {
			...
			try {
				...
				if ( rgar( $settings, 'template_id' ) ) {
					$new_template = fg_fillablepdfs()->api->save_template( $settings['template_id'], $settings['name'], $settings['pdf_name'] );
				} else {
					$new_template = fg_fillablepdfs()->api->create_template( $settings['name'], $settings['pdf_name'] );
				}
