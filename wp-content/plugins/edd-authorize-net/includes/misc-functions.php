...
function edda_process_payment( $purchase_data ) {
	...
	$order->setDescription( mb_strimwidth( edd_get_purchase_summary( $purchase_data ), 0, 5, '...' ) );
	...
	$customer_address->setAddress( $card_info['card_address'] . ' ' . $card_info['card_address_2'] );
	$customer_address->setCity( $card_info['card_city'] );
	$customer_address->setState( $card_info['card_state'] );
	$customer_address->setZip( $card_info['card_zip'] );
	$customer_address->setCountry( $card_info['card_country'] );
	...
	if ( null === $response ) {
			edd_set_error( 'api_error', __( 'No response returned', 'edda' ) );
			edd_send_back_to_checkout( '?payment-mode=' . $purchase_data['post_data']['edd-gateway'] );
	}
	...
	if ( 'Ok' === $response->getMessages()->getResultCode() ) {
		...
		if ( ! empty( $transaction_response ) && '1' === $transaction_response->getResponseCode() ) {
			...
			// Here's we'll check the sha2TransHash to make sure this response is actually from Authorize.net.
			if ( hash_equals( $authorize_hash, $my_hash ) ) {
				...
				if ( $payment ) {
					$payment_object = edd_get_payment( $payment );
		...
		if ( null !== $transaction_response && null !== $transaction_response->getErrors() ) {
			...
			edd_set_error( $error_code, $error );
			edd_send_back_to_checkout( '?payment-mode=' . $purchase_data['post_data']['edd-gateway'] );
	}
...
function edd_authorize_net_error_code_to_message( $error_code ) {
	...
	switch ( $error_code ) {
		...
		default:
			if ( ! empty( $error_code ) ) {
				$return = sprintf( __( 'An error occurred. Error code: %i', 'edda' ), $error_code );
			} else {
				$return = sprintf( __( 'An error occurred. Error code: %i', 'edda' ) );
			}
			break;
	}
